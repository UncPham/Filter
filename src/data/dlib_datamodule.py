...
class DlibDataset(Dataset):
    ...
    @staticmethod
    def annotate_image_triplot(image: Image, landmarks: np.ndarray) -> Image:
        draw = ImageDraw.Draw(image)
        tri = Delaunay(landmarks)
        
        for simplex in tri.simplices:
            pts = landmarks[simplex]
            draw.line([tuple(pts[0]), tuple(pts[1]), tuple(pts[2]), tuple(pts[0])], fill=(0, 255, 0), width=2)
